...
class GoGameAI:
    ...
    Attributes:
        # where the args are stored 
        firstAI (str): this is where we store the executable that will be used for the first net
        secondAI (str): this is where we store the executable that will be used for the second net
        typeFirstAI (str): type of the first executable; currently supported: 'LZ' for leeelazero, 'SAI' for SAI, and 'KG' for Katago
        typeSecondAI (str): type of the second executable; currently supported: as for typeFirstAI
        fileSGF (str): this is where we store the argument fileSGF
    ...
    """
    #constructor
    def __init__(self, firstAI, secondAI,typeFirstAI, typeSecondAI, firstPlayer,secondPlayer,colorFirst,fileSGF,startingBoard):
        #stable attributes 
        ...
        self.processOrd={} #dictonary of the two processes that will be launched
    ...
    def AIProcess(self,typeAI, AI,player,literal): #AIProcess(typeAI, AI, player,literal)
        """
        AIProcess is a method opening a process of the softwareAI of type typeAI, using a specified set of parameters, and importing the currentBoard; it returns an instance of subprocess.Popen()

        Args: 
            player (str): net to be used when opening the process (either firstPlayer or secondPlayer) 
        ...
        """
        if typeAI!='SAI' and typeAI!='LZ' and typeAI!='KG':
            errormessage = "ERROR: the types of AI currently supported are SAI, LZ and KG: "+typeAI+" is not supported"
            raise Exception(errormessage)
        if typeAI=='SAI' or typeAI=='LZ':
            #clean literal
            ...
            if checkq:
                print("WARNING: the '-q' switch is incompatible with the functionalities of GoGameAI with SAI or LZ; I am dropping it")
            if checkg==False:
                print("WARNING: the '-g' switch is necessary for the functionalities of GoGameAI with SAI or LZ; I am adding it")
                literalnew.append('-g')
            literalclean=" ".join(literalnew)    
            lineLeela=AI+" -w "+ player+" "+ literalclean
            print('opening process of leelaz for a net of type',typeAI,'with command:',lineLeela)
            ...
            processPlayer=subprocess.Popen(lineLeela,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,universal_newlines=True,shell=True)
        if typeAI=='KG':
            lineKG= AI+' gtp -model '+player+' -config default_gtp.cfg -override-config '+literal
        ...
        if self.currentBoard!='':
            line='loadsgf '+self.currentBoard+'\n'
            processPlayer.stdin.write(line)
            processPlayer.stdin.flush()
        return processPlayer
    #method emptying the stderr of a leelaz process
    ...
    #method closing a leelaz process 
    ...
    def incorporateMove(self,AIProcess,move,colorMove):
        ...
        AIProcess.stdin.write(line)
        AIProcess.stdin.flush()
        #method incorporating a move in a leelaz process
    ...
    def printSGF(self,AIProcess,typeAI):
        """
        ...
        """
        print('printing',self.fileSGF)
        with open(self.fileSGF,"w") as sgf: 
    ...
    def AIOrdProcess(self,ordplayer,literal):
        """
        AIOrdProcess invokes AIProcess based on the player number 1 or 2, according to its argument
        ...
        AIProcess=self.AIProcess(typeAI, AI, player,literal)
    ...
    def startGame(self,literalFirst,literalSecond,komi):
        ...
        self.setKomi(self.processOrd['1'],komi)
        self.setKomi(self.processOrd['2'],komi)
    ...
    def printSGFOrdProcess(self,ordplayer):
        ...
        self.printSGF(AIProcess,typeAI)
   #close the two players' processes
    def closeTwoPlayerProcesses(self):
        ...
        """
        for ord in ['1','2']:
            self.closeAIProcess(self.processOrd[ord])
    ...
    def moveNext(self):
        ...
        move=self.generateMove(thisProcess,thisColor)
        if move=="resign":
        ...
        self.incorporateMove(self.processOrd[self.otherOrdPlayer[thisOrdPlayer]],move,thisColor)
    ...
    def playDefault(self,literalFirst,literalSecond,komi):
        """
        ...
        while True:
            ...
            if self.gameStatus=='completed':
                break      
        #print the SGF
        self.printSGF(thisProcess,self.typeAIOrdPlayer[thisOrdPlayer])
    ...
    def playWithVariableAgent(self,literalFirst,literalSecond,ordPlayerVariableAgent,threshold,parmu,parlambda,komi):
        """
        ...
        while True:
            ...
            if thisOrdPlayer==ordPlayerVariableAgent:
                ...
                params=self.extractParametersCurrentBoard(thisProcess)
                if 'winrate' in params.keys():
                    if params['winrate']>threshold:
                        self.setOption(thisProcess,'lambda',parlambda)
                        self.setOption(thisProcess,'mu',parmu)
                    else:
                        self.setOption(thisProcess,'lambda',0)
                        self.setOption(thisProcess,'mu',0)
                else:
